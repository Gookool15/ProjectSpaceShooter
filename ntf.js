...
function displayScore() {
	...
	ctx.fillText(score, 625, 30);
	ctx.fillText('Lives:', 10, 30);
	ctx.fillText(lives, 100, 30);

	if (!gameStarted) {
  		ctx.font = 'bold 50px Orbitron';
  		ctx.fillText('Alien Space Shooter', WIDTH/2-250,HEIGHT/3);
  		ctx.font = 'bold 27px Orbitron';
  		ctx.fillText('Click to Play', WIDTH/2-100, HEIGHT/2+30);
  		ctx.fillText('Use arrow keys to move', WIDTH/2-175, HEIGHT/2+70);
  		ctx.fillText('Use the spacebar key to shoot', WIDTH/2-220,HEIGHT/2+110);
	}

	if (!alive) {
  		ctx.fillText('Game Over!',245,HEIGHT/2);
  		ctx.fillText('Click anywhere to play again',WIDTH/2-200,HEIGHT/2+35);
